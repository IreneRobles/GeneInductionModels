...
function A_matrix_induction_0mRNASTART(parameters, N)
...
function ODEsolve_induction_0mRNASTART(prms; 
    ...
    f = parameters -> A_matrix_induction_0mRNASTART(parameters, N)
    ...
    mRNA = [a[1:N] .+ a[N+1:2N] .+ a[2N+1:3N] .+ append!([a[end]], zeros(N-1)) for a in sol.u]
